...
class AuctionQueryRepositoryImplTest extends DataJpaTestSupport {
    ...
    void findByProductCategories() {
        //given
        ProductCategory notPreferredCategory = productCategoryRepository.save(
            ProductCategory.from("스포츠/레저"));
        Auction auction1 = AuctionFixture.auction(category1);
    ...
    void updateAuctionStatus() {
        ...
        Auction endedAuctionWithBidder = AuctionFixture.auction(category1,
            today.minusDays(1)); // 마감 일자(endDate)
        Auction endedAuctionWithOutBidder = AuctionFixture.auction(category1,
            today.minusDays(1)); // 마감 일자(endDate)
        ...
        auctionRepository.saveAll(
            List.of(endedAuctionWithBidder, endedAuctionWithOutBidder, biddingAuction1,
                biddingAuction2));
        ...
        Auction savedEndedAuctionWithBidder = auctionRepository.findById(
            endedAuctionWithBidder.getId()).orElseThrow();
        Auction savedEndedAuctionWithOutBidder = auctionRepository.findById(
                endedAuctionWithOutBidder.getId())
            .orElseThrow();
        Auction savedBiddingAuction1 = auctionRepository.findById(biddingAuction1.getId())
            .orElseThrow();
        Auction savedBiddingAuction2 = auctionRepository.findById(biddingAuction2.getId())
            .orElseThrow();

        //then
        assertAll(
            () -> assertThat(savedEndedAuctionWithBidder.getStatus()).isEqualTo(
                AuctionStatus.TRADING),
            () -> assertThat(savedEndedAuctionWithOutBidder.getStatus()).isEqualTo(
