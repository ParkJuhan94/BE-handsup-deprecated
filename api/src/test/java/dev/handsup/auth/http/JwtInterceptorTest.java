...
@Tag("unit")
@DisplayName("[JwtInterceptor 단위 테스트]")
...
class JwtInterceptorTest {
    ...
    void shouldPassWithoutTokenWhenNoAuthAnnotationIsPresent() {
        ...
        // then
        assertThat(jwtInterceptor.preHandle(mockHttpServletRequest, response, handlerMethod))
            .isTrue();
    }
    ...
    void shouldThrowExceptionWhenTokenIsMissing() {
        assertThat(jwtInterceptor.preHandle(mockHttpServletRequest, response, handlerMethod)).isFalse();
    }
    ...
    void shouldThrowExceptionWithInvalidToken() {
        // given
        mockHttpServletRequest.addHeader(HttpHeaders.AUTHORIZATION, "Bearer invalidToken");
