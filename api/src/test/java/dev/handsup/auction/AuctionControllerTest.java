...
class AuctionControllerTest extends ApiTestSupport {
    ...
    void registerAuction() throws Exception {
        RegisterAuctionRequest request = RegisterAuctionRequest.of("거의 새상품 버즈 팔아요", DIGITAL_DEVICE,
            10000,
            LocalDate.parse("2022-10-18"), ProductStatus.NEW.getLabel(),
    ...
    void registerAuctionFails() throws Exception {
        ...
        RegisterAuctionRequest request = RegisterAuctionRequest.of("거의 새상품 버즈 팔아요",
            NOT_EXIST_CATEGORY, 10000,
            LocalDate.parse("2022-10-18"), ProductStatus.NEW.getLabel(),
            PurchaseTime.UNDER_ONE_MONTH.getLabel(),
        ...
        mockMvc.perform(post("/api/auctions").header(AUTHORIZATION, "Bearer " + accessToken)
            ...
            .andExpect(jsonPath("$.message").value(
                AuctionErrorCode.NOT_FOUND_PRODUCT_CATEGORY.getMessage()))
            .andExpect(
                jsonPath("$.code").value(AuctionErrorCode.NOT_FOUND_PRODUCT_CATEGORY.getCode()));
    }
    ...
    void getAuctionDetail() throws Exception {
        ...
        //when, then
        mockMvc.perform(
                get("/api/auctions/{auctionId}", auction.getId()).contentType(APPLICATION_JSON))
            .andExpect(status().isOk())
    ...
    void getRecommendAuctionsWithFilter() throws Exception {
        ...
        //when
        mockMvc.perform(get("/api/auctions/recommend").param("sort", "END_DATE")
                .param("si", si)
                .param("gu", gu)
                .param("dong", dong1)
                .contentType(APPLICATION_JSON))
    ...
    void getRecommendAuctionsWithOutFilter() throws Exception {
        ...
        //when
        mockMvc.perform(
                get("/api/auctions/recommend").param("sort", "CREATED").contentType(APPLICATION_JSON))
            .andExpect(status().isOk())
    ...
    void getRecommendAuctions_fails2() throws Exception {
        mockMvc.perform(
                get("/api/auctions/recommend").contentType(APPLICATION_JSON).param("sort", "NAN"))
            .andDo(MockMvcResultHandlers.print())
            .andExpect(
                jsonPath("$.message").value(AuctionErrorCode.INVALID_SORT_INPUT.getMessage()))
    ...
    void getUserPreferredCategoryAuctions() throws Exception {
        ProductCategory productCategory2 = productCategoryRepository.save(
            ProductCategory.from("생활/주방"));
        ProductCategory notPreferredProductCategory = productCategoryRepository.save(
            ProductCategory.from("티켓/교환권"));
